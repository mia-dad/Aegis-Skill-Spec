...
### 5.1 CognitiveSkill 变量作用域说明表格

CognitiveSkill 通过节点的 `as`/`to`/`collect` 等机制显式控制变量输出到上下文的作用域。

| 节点类型 | `as` 用途 | 作用域 | 输出变量说明 |
|---------|----------|--------|-------------|
| `select` | 无 `as` | 全局 | 通过 `output:` 字段名直接输出到上下文 |
| `assign` | 使用 `to:` 字段 | 全局 | `to:` 指定的变量名存入上下文 |
| `merge` | `as:` 合并结果名 | 全局 | `as:` 指定的变量名存入上下文 |
| `foreach` | `as:` 迭代变量（局部）<br>`collect:` 收集变量（全局） | 混合 | `as:` 仅循环内可用<br>`collect:` 循环结束后全局可用 |
| `loop` | `as:` 最终结果名 | 全局 | `as:` 指定的变量名存入上下文 |

**注意事项：**

1. **`as` 关键字的多重语义**：
   - `foreach` 中 `as:` 定义迭代变量（局部作用域）
   - `merge` 和 `loop` 中 `as:` 定义输出变量（全局作用域）
   - 需根据节点类型理解 `as` 的具体语义

2. **变量引用方式**：
   - CognitiveSkill 的节点输出通过 `as`/`to`/`collect` 映射后，直接通过变量名引用
   - 不需要步骤名前缀（与 AtomicSkill 不同）
...
- type: select
  capability: reasoning
  output:
    Conclusion: reasoning_result    # 写入 context["reasoning_result"]

# assign 节点：通过 to: 写入上下文
- type: assign
  from: reasoning_result
  to: analysis                        # 写入 context["analysis"]

# foreach 节点：as 是局部，collect 是全局
- type: foreach
  items: sub_questions
  as: sq                    # 局部：循环内可用
  collect: all_answers      # 全局：循环结束后可用
  do:
    - type: select
      output:
        SubAnswer: sub_answer
  # 引用：sub_answer 会被收集为 all_answers = [sub_answer1, sub_answer2, ...]

# merge 节点：as 定义合并结果
- type: merge
  sources: [result1, result2]
  as: merged_result         # 写入 context["merged_result"]

# 后续节点直接引用变量名
- type: select
  input:
    Query: analysis         # 直接引用，无需步骤名前缀
  output:
    Answer: final_answer
```
