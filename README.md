...
**当字段类型为 `array` 时，必须通过 `items` 完整定义其元素结构，直到最末级节点的类型为基本类型（`string`、`number`、`boolean`、`datetime`、`resource`）只有该技能对于该数组类型的内部细节和类型暂时无法获知时，才可以将本级作为定义时的最末级。**
...
# ✅ 推荐：items 下的 object 定义了子字段直到基本类型
records:
  type: array
  items:
    name:
